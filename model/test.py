...
########################################################################
webCamFeed = True
pathImage = "F:\\Project2_Graduation\\optical-mark-recognition\\images\\56427.jpg"
...
while True:
    ...
    try:
        ...
        if biggestPoints.size != 0 :
            ...
            cv2.imshow("counters",imgBigContour)
            cv2.waitKey(0)
            pts1 = np.float32(biggestPoints)  # PREPARE POINTS FOR WARP
            pts2 = np.float32([[0, 0], [widthImg, 0], [0, heightImg], [widthImg, heightImg]])  # PREPARE POINTS FOR WARP
            matrix = cv2.getPerspectiveTransform(pts1, pts2)  # GET TRANSFORMATION MATRIX
            imgWarpColored = cv2.warpPerspective(img, matrix, (widthImg, heightImg))  # APPLY WARP PERSPECTIVE
            cv2.imshow("showAnswerInImage",imgWarpColored)
            cv2.waitKey(0)
            ...
            #imgWarpblure=cv2.GaussianBlur(imgWarpGray, (5, 5), 1)
            #new_image = cv2.Laplacian(imgWarpColored, cv2.CV_64F)
            #new_image2=imgWarpGray + new_image
            #plt.imshow(imgWarpGray + new_image, cmap='gray'), plt.title('Resulting image')
            ...
            cv2.imshow("ِAnswerImagegray", imgWarpGray)
            cv2.waitKey(0)

            #equ = cv2.equalizeHist(imgWarpGray)
            #res = np.hstack((imgWarpGray, equ))  # stacking images side-by-side
            #cv2.imshow("eq",equ)
            #cv2.waitKey(0)

            #imgThresh = cv2.adaptiveThreshold(imgWarpGray, 170, 250, cv2.THRESH_BINARY_INV)[1]  # APPLY THRESHOLD AND INVERSE
            new_img=cv2.adaptiveThreshold(imgWarpGray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY, 11, 2)
            cv2.imshow("AdaptiveThreeShold", new_img)
            cv2.waitKey(0)

            imgThresh = cv2.threshold(new_img, 170, 250, cv2.THRESH_BINARY_INV)[1]  # APPLY THRESHOLD AND INVERSE
            cv2.imshow("BinaryTHreeShold", imgThresh)
            cv2.waitKey(0)
            ...
            boxes = utlis.splitBoxes(imgThresh,25)  # GET INDIVIDUAL BOXES
            #print(len(boxes))
